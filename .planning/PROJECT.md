...
**Goal:** 在保持 MacStatus 轻量菜单栏体验的前提下，为 MacBook Pro 增加风扇转速与关键温度只读监控，并修复网络数值变化导致下拉窗布局抖动的问题。（风扇安全控制于 2026-06-30 撤销，见下。）

**Target features:**
- 热状态监控 — 显示整机 CPU/SoC 等关键温度，尽量读取 GPU、电池、SSD 等重要传感器；不可用时优雅显示 N/A。
- 风扇监控 — 在 MacBook Pro 上显示当前风扇 RPM、风扇数量、可用/不可用状态，非支持机型优雅降级。
- ~~风扇安全控制~~ — 已于 2026-06-30 撤销（用户取消风扇硬件改动功能）；可在后续里程碑重新评估。
...
- 无活跃需求。v3.0 只读监控范围（温度/风扇/布局）已交付；风扇安全控制已撤销（见下）。里程碑保持开放。
...
- 风扇安全控制（手动 opt-in 写入/读回验证/恢复自动）— 2026-06-30 从 v3.0 撤销（用户取消风扇硬件改动功能），可后续里程碑重提；规划产物归档于 .planning/cancelled/
...
| 风扇控制必须先研究再落地 | SMC 写入涉及硬件安全和机型差异，不能直接按猜测写值 | — Cancelled（功能撤销） |
| v3.0 控制边界为安全控制而非完整自动曲线 | 先交付可恢复、可限制、可验证的用户控制闭环 | — Cancelled（功能撤销） |
| 取消 v3.0 风扇硬件控制（Phase 13/14） | 用户决定撤销风扇硬件改动功能；只读监控已满足核心价值，写入路径风险/成本不再纳入本里程碑 | ✓ Descoped 2026-06-30 |

---

*Last updated: 2026-06-30 — Phases 13 & 14 (fan control write path) cancelled; v3.0 kept open*
