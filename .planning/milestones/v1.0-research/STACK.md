...
This is a native macOS app — no package manager installation:
...
xcodebuild -project MacStatus.xcodeproj -scheme MacStatus -configuration Release build
```

### What goes in the Xcode project:

```
MacStatus.xcodeproj          # Single target, no workspace needed
├── MacStatus/
│   ├── AppDelegate.swift    # @main, NSApplicationDelegate, NSStatusBar setup
│   ├── Info.plist           # LSUIElement = YES
│   ├── Monitors/
│   │   ├── CPUMonitor.swift      # host_processor_info reader
│   │   ├── MemoryMonitor.swift   # host_statistics64 reader
│   │   ├── GPUMonitor.swift      # IOKit IOAccelerator reader
│   │   └── NetworkMonitor.swift  # getifaddrs reader
│   ├── Views/
│   │   ├── MenuBarView.swift     # NSHostingView or NSAttributedString display
│   │   └── SettingsView.swift    # SwiftUI preferences
│   ├── Models/
│   │   └── SystemStats.swift     # Shared data model
│   └── Helpers/
│       └── Formatters.swift      # Byte formatting, percentage formatting
└── MacStatusTests/
    └── (unit tests)
```
...
**macOS 26 (Tahoe) privacy requirement:**
...
**If Apple Silicon only (no Intel support needed):**
...
**If GPU monitoring proves unreliable on some Macs:**
...
---
*Stack research for: macOS Status Bar System Monitor*
*Researched: 2026-05-14*
