...
**Iteration:** 2（累计两轮，共 7 个 in-scope findings 全部修复）

**Summary:**
- Findings in scope: 7（迭代 1：CR-01、CR-02、WR-01、WR-02、WR-03、WR-04；迭代 2：WR-01-iter2）
- Fixed: 7
- Skipped: 0

构建验证：迭代 2 修复应用后执行 `xcodebuild` **BUILD SUCCEEDED**，无编译错误。
...
### WR-01（迭代 2）: `updateTitle()` 在所有 active metric 均产生 nil segment 时输出空字符串而非占位符

**Files modified:** `MacStatus/MacStatus/UI/StatusBarManager.swift`
**Commit:** `780730e`
**Applied fix:** 在 `updateTitle()` 的组合循环结束后新增 `result.length == 0` 检测。若所有 active metrics 均返回 `nil` segment（如 `enabledMetrics = [.battery]`），则设置 `button.title = "◆"` 并 `return`，避免将空串赋给 `button.attributedTitle` 导致菜单栏显示空白，保持注释中"never leaves the status bar blank"的不变量。

---

## Skipped Issues

无跳过项——两轮共 7 个 in-scope findings 均已成功修复并通过构建验证。
